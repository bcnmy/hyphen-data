...
export default function SearchBar(props) {
    ...
    const [selectedNetwork, setSelectedNetwork] = useState(
        config.supportedChainsArray[0].chainId
    );
    ...
    const handleHashSearch = () => {
        if (searchText && searchText.length > 0) {
            dispatch(updateSearchState({ searchText: searchText }));
            dispatch(
                updateRootState({ currentPage: config.PAGE.TRANSFER_DETAILS })
            );
        }
    };
