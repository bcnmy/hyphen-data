...
let getDepositHash = (depositHash) => {
    return new Promise(async (resolve, reject) => {
        try {
            let data = await ethereumGraphClient.query({
                query: gql(`
                query {
                  userTransferInfos(where: {depositHash: "${depositHash}"}) {
                        id
                        depositHash
                        LpFee
                        amount
            ...
            resolve(data.data.userTransferInfos[0]);
        } catch(error) {
            reject(error);
        }
    })
...
let getTransferData = (exitHash) => {
    return new Promise(async (resolve, reject) => {
        try {
            let data = await ethereumGraphClient.query({
                query: gql(`
                query {
                    fundsSentToUsers(where: {id: "${exitHash}"}) {
                        id
                        asset
                        amount
                        target
...
function App() {
    ...
    let searchDepositHash = async () => {
        let userTransferInfos = await getDepositHash(depositHash);
        let response = {}
        response.exitHash = userTransferInfos.id
        response.depositHash = userTransferInfos.depositHash
        response.amountTransferred = userTransferInfos.amount
        response.feeEarned = userTransferInfos.LpFee
        ...
        let transferData = await getTransferData(userTransferInfos.id);
        response.exitHash = userTransferInfos.id
        response.depositHash = userTransferInfos.depositHash
        response.tokenAddress = transferData.asset
        response.amountReceived = transferData.amount
        response.receiver = transferData.target
